 ...
 * plugins/style/ (the acme.pitch-deck pattern; howto-zod-to-fdpm-plugin
 * §11 `example:ci-snapshot-gate`, `principle:schema-change-implies-version-bump`).
 *
 * Outputs (all under plugins/style/):
 ...
 *   capabilities/<Entity>.capabilities.json   (fifteen, one per entity)
 *   fdpm-plugin.json                          (scaffold + one cap:renderer per entity)
 *
 * index.ts, ingest.ts, invariants.ts and renderers/ are author-owned and
 * NOT regenerated.
 *
...
function buildPlanned(): { files: Map<string, string> } {
  ...
  const schemaSrc = readFileSync(join(PLUGIN_DIR, "schemas", "style.ts"), "utf8");
  ...
  files.set(
    ...
    stableStringify({
      ...
      sources: ["schemas/style.ts", "sidecar.ts"],
    }) + "\n",
  );
...
function buildBaseManifest(
  ...
  const tmp = join(tmpdir(), `fdpm-style-base-${process.pid}`);
  rmSync(tmp, { recursive: true, force: true });
  mkdirSync(tmp, { recursive: true });
  writePluginScaffold(result, {
    outputDir: tmp,
    pluginName: "Visual Style Definition",
    pluginDescription:
      "StyleDefinition 3.1.0, auto-generated from schemas/style.ts via @fdpm/zod-bridge. Fifteen entities as primitives with field-level Zod validators, plus ten author-declared relation types (grammar composition, rule declaration, compliance linkage, exemplar citation, reference buckets, movement lineage) merged by finalizeProfile.",
