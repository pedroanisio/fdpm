/**
 * Local field-builder helpers for the knowledge-cartridge plugin.
 *
 ...
 *
 * `jsonField` is kept from the sibling set because a cartridge stores one
 * genuinely open payload — a kc:Constant's worked example, whose shape is
 * decided by the craft rather than by this profile.
 */
import type {
  FieldDefT,
  FieldValidation,
 ...
 * distinguish a payload it must `JSON.parse` from ordinary prose.
 */
