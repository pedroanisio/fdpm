...
> `package.json` reads `1.3.0`, aligned with the runtime's `HOST_VERSION`
> (bumped to 1.3.0 with SPEC-CORE 1.3 in 67b3401; the manifest followed on
> 2026-09-04 so `npm run public:check` reports zero findings), as the first
> public-release candidate. The earlier `1.1.0` and `1.2.0` values were
> development versions rather than shipped releases, so there is still no
> truthful historical boundary at which to split this section.
...
is a 50-instruction test set against `profile:planning:0.1` (12 simple, 12
multi-step, 10 batch, 8 ambiguity, 8 refusal), generated by
`scripts/build-cold-agent-test-set.ts`; every instruction carries a fixture
(executed through the real `fdpm-mcp`), assertions over the terminal workbook
state, the destructive scope it authorises, and a reference solution that is
both the human baseline and the set's self-check. The runner spawns one server
per instruction and arm, drives a model through a bounded manual tool loop
(`@anthropic-ai/sdk`, adaptive thinking, no refusal fallbacks so one snapshot
answers a run), and scores from the Host projection, the operation log and
`mcp-audit.jsonl` — never from the model's text — on the four README criteria:
terminal state, replay from empty, destructive scope, and write calls ≤ 2 × the
reference. The three arms are client-side views of one server: the core
catalog minus the profile-discovery tools; plus `initialize.instructions` and
the resource surface; plus `prompts/list` / `prompts/get`. Per-verb plugin tools
and the PURPOSE.md discovery tools are still unbuilt, so the arms measure what
ships. The report computes the 15-percentage-point arm 3 − arm 2 differential
and the acceptable-rate kill criterion. `npm run eval:reference` proves every
reference passes all four criteria (50/50 on 2026-09-04); `npm run
eval:cold-agent -- --model <id>` is the measurement, which has not yet been
run. Documented in `docs/eval/COLD-AGENT-EVAL.md`; `@anthropic-ai/sdk` is a
devDependency — it replaces the HTTP client, retry and typed-error code the
driver would otherwise carry.

#### Disclaimer 2.0 and the Silent Acceptance banner

`DISCLAIMER.md` is version 2.0: the v1 notice ("may be invalid, erroneous, or a
hallucination") is replaced by a verification statement, the same edit the
Silent Acceptance specification (formerly PALS's Law; v2.0.0, DOI
10.5281/zenodo.19401266) made in its own revision, and commitment 8 declares
the verification boundary every model-output consumer carries. The root README
is the VeraFormX product overview with the paper's short-form banner; the
former README is `docs/architecture/DESIGN.md`. Document banners that read
"PALS's Law" now name Silent Acceptance.

#### Packed-install smoke matches the binary's refusal wording

`scripts/smoke-packed-install.mjs` still expected the installed `fdpm-mcp`
shim to refuse an HTTP flag with "HTTP transport is not supported"; the
binary has said "this binary speaks stdio only" since 67b3401, so
`npm run smoke:pack` (the CI smoke step) failed on every run after
2026-08-31. The script now matches the stable clause of the current message
and prints the shim's stderr when it does not.
