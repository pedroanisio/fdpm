...
describe("§5.4 graph operations", () => {
  it("core-graphops-clone-001: clone copies primitives and relations under a new id", async () => {
    ...
    const createOp = log.find((o) => o.kind === "project.create")!;
  ...
  it("core-graphops-clone-002: clone with target id collision yields 409 conflict", async () => {
    ...
    await expect(
      cloneProject(host, "p1", { target_project_id: "p2", target_project_name: "Dup" }),
    ).rejects.toThrow(/exists|conflict/);
  });

  it("core-graphops-split-003: profile with no partition unit rejects :split", async () => {
    const host = await newHost();
    await host.createProject({ project_id: "p1", name: "P1", profile_id: "core:empty" });
    await expect(
  ...
  it("core-graphops-split-001: split partitions into N projects, drops cross-partition relations", async () => {
    const host = await newHost();
    await host.createProject({ project_id: "p1", name: "P1", profile_id: "test:demo" });
    await host.createPrimitive("p1", {
      id: "section:a",
    ...
    const result = await splitProject(host, "p1", {
      partition: [
        { target_project_id: "p1-a", target_project_name: "A", sections: ["section:a"] },
        { target_project_id: "p1-b", target_project_name: "B", sections: ["section:b"] },
      ],
      cross_partition_relations: "drop",
    });
  ...
  it("core-graphops-split-002: section appearing in two partition entries rejected", async () => {
    ...
    await expect(
      splitProject(host, "p1", {
        partition: [
          { target_project_name: "X", sections: ["section:a"] },
          { target_project_name: "Y", sections: ["section:a"] },
        ],
        cross_partition_relations: "drop",
      }),
