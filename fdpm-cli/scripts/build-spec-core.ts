...
const terms: Array<[string, string, string?]> = [
  ...
  [
    "Instance",
    "A populated `PrimitiveInstance` or `RelationInstance` belonging to a Project (§5).",
  ],
  [
    "Verification gate",
...
const invariants: PrimitiveSpec[] = [
  ...
  },
  {
    ...
    fields: {
      ...
      statement:
        "The shell remains useful with no plugins installed (§10.2). `core:empty` (§1.5) makes \"a project with any registered profile can be opened\" satisfiable.",
      enforcement: "ci_check",
      scope_ref: "§10.2",
    },
  },
...
const acceptances: PrimitiveSpec[] = [
  ...
  ac(15, "core-endpoint-003", "After §19 migration, requests to pre-migration paths (`/api/narrative/*`, `/api/spec_parser/*`, `/api/projects/{id}/export.pdf`) return 404 — no redirect, no shim. The contract test enumerates these paths and asserts their absence from OpenAPI.", "open"),
  ac(16, "core-fe-baseline-001", "The SPA, run with zero plugins, exercises §10.2 in an end-to-end browser test (including default print/preview render and `core:empty` project open).", "open"),
  ...
  ac(27, "core-graphops-split-001", "`POST /projects/{id}:split` with a valid 2-way partition produces two new projects, deletes the source, drops cross-partition relations, and returns the dropped list. Audit log captures `project.split` + per-new-project `project.create` + per-dropped `relation.drop` records under one `request_id`.", "open"),
  ac(28, "core-graphops-split-002", "Each refused-input case in §5.4.1 returns the documented status/category. Atomicity test: a partition that triggers a per-primitive validation failure leaves the source project unchanged.", "open"),
  ...
  ac(30, "core-graphops-clone-001", "`POST /projects/{id}:clone` produces a project with the same `profile_id`, copies all primitives and relations and templates and test suites, does NOT copy `SuiteRunReport`s, and emits one `project.create` audit record with `cloned_from` evidence.", "open"),
  ...
  ac(43, "core-eventsource-004", "`GET /projects/{id}/log` honours all query filters and respects `read:audit`.", "open"),
  ac(44, "core-eventsource-005", "`GET /projects/{id}/at?revision=N` returns state byte-equal to `replay(log[:N+1])` for every N.", "open"),
...
const futureWork: PrimitiveSpec[] = [
  ...
  fw("spec:fw:cross-project-federation", "Cross-project federation (instances referencing other projects)", "§5.3 forbids cross-project references in v1.1. Federation is a future SPEC.", null),
  ...
  fw("spec:fw:cross-partition-preserve", "`cross_partition_relations: \"preserve\"` for `:split`", "Requires cross-project references in the meta-model, which §5.3 forbids in v1.1.", null),
  ...
  fw("spec:fw:project-merge", "Project merge / re-join (inverse of `:split`)", "Once split, projects are independent in v1.1.", null),
  fw("spec:fw:cap-project-event", "`cap:project-event` (§5.4.4)", "Letting plugins subscribe to operation-log events without polling. The natural \"free feature\" event sourcing unlocks; deferred to a future SPEC.", null),
  ...
  fw("spec:fw:primitive-level-split-clone", "Primitive-level split or clone", "Different surface from project-level (§5.4); rejected for v1.1 because no user story requires it.", null),
...
const conformance: PrimitiveSpec[] = [
  conf(1, "Operation log canonicality", "Append an operation; query `GET /projects/{id}/log`; assert the operation is present with monotone revision.", "Log returns the appended operation; revision strictly greater than previous max."),
  ...
  conf(5, "Zero-plugins baseline", "Run the SPA against a host with no plugins installed; exercise §10.2 bullets.", "Login, project list, project open against `core:empty`, default form/card/panel render, default print renderer produce output, plugin admin shows \"no plugins installed\"."),
];
...
const revisions: PrimitiveSpec[] = [
  ...
  rev("spec:rev:1-0-4", "1.0.4", "2026-05-04", "Final-pass clarification",
    ...
    ["0", "9.7.5"], "editorial"),
  rev("spec:rev:1-0-3", "1.0.3", "2026-05-04", "Core graph operations (project-level)",
