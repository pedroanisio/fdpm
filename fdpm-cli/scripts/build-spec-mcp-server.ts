...
const terms: Array<[string, string, string?]> = [
  ...
  [
    "Operation envelope",
    "The Core's record of a single state-mutating operation: id, kind, project_id, before/after fingerprints. Surfaced in MCP success responses so clients can correlate.",
  ],
  [
    "stdio transport",
...
const scenarios: PrimitiveSpec[] = [
  {
    ...
    fields: {
      ...
      stimulus: "Calls `fdpm.project.delete` with a valid project_id.",
      environment:
        "Server started with default config (destructive tools NOT enabled).",
      artifact: "MCP server's authorization gate.",
      response:
        "Server returns `isError: true` with `structuredContent.error.category = \"permission\"` and `evidence.reason = \"destructive_disabled\"`; no operation is appended to the log; no project state is mutated; the call is recorded in the audit log.",
      response_measure:
  ...
  },
  {
    ...
    fields: {
      ...
      source: "LLM agent.",
      stimulus:
        "Calls `fdpm.primitive.search` against a representative project on the standard fixture set.",
...
const toolEntries: Array<{
  ...
  {
    id: "spec:tool:project-list",
    tool_name: "fdpm.project.list",
    tier: "read_only",
    exposure: "always",
    description: "List all projects in the current data dir.",
    backed_by: "host.listProjects()",
  },
  {
    id: "spec:tool:project-get",
    tool_name: "fdpm.project.get",
    tier: "read_only",
    exposure: "always",
    description: "Fetch a project's full slice (project + primitives + relations + templates).",
    backed_by: "host.getProject(id)",
  },
  {
    id: "spec:tool:project-diff",
    tool_name: "fdpm.project.diff",
    tier: "read_only",
    exposure: "always",
    description: "Diff a project against another revision/snapshot.",
  ...
  {
    id: "spec:tool:validate-project",
    tool_name: "fdpm.validate.project",
    tier: "read_only",
    exposure: "always",
    description: "Run the §7 pipeline against a project without persisting any change.",
  ...
  },
  {
    ...
    tier: "read_only",
    exposure: "always",
    description: "Return the bounded tail of a project's operation log.",
  ...
  {
    id: "spec:tool:project-create",
    tool_name: "fdpm.project.create",
    tier: "validating_write",
    exposure: "default_on",
  ...
  },
  {
    ...
    tier: "validating_write",
    exposure: "default_on",
    description: "Create a primitive in a project.",
    backed_by: "host.createPrimitive(project_id, primitive)",
  },
  {
    ...
    backed_by: "host.replacePrimitive(project_id, primitive)",
  },
  {
    ...
    backed_by: "host.patchPrimitive(project_id, patch)",
  },
  {
    ...
    backed_by: "host.fieldPatchPrimitive(project_id, payload)",
  },
  {
    ...
    backed_by: "host.createRelation(project_id, relation)",
  },
  {
    ...
    backed_by: "host.replaceRelation(project_id, relation)",
  },
  {
    ...
    backed_by: "host.patchRelation(project_id, patch)",
  },
  {
    ...
    backed_by: "host.reorder(project_id, scope_id, ordering)",
  },
  {
    ...
    backed_by: "host.reparent(project_id, payload)",
  },
  {
  ...
  {
    id: "spec:tool:project-delete",
    tool_name: "fdpm.project.delete",
    tier: "destructive",
    exposure: "opt_in",
    description: "Delete a project.",
    backed_by: "host.deleteProject(project_id)",
  },
  {
    ...
    tier: "destructive",
    exposure: "opt_in",
    description: "Delete a primitive.",
    backed_by: "host.deletePrimitive(project_id, id)",
  },
  {
    ...
    backed_by: "host.deleteRelation(project_id, id)",
  },
  {
...
const conformance: PrimitiveSpec[] = [
  ...
  },
  {
    ...
    fields: {
      ...
      procedure:
        "While the MCP server is running, run a `fdpm` CLI command that mutates the project. Then issue a Tier 2 MCP call against the same project.",
      expected:
...
const futureWork: PrimitiveSpec[] = [
  ...
  },
  {
    ...
    fields: {
      ...
      description:
        "Native MCP resources for project state (not just tools) and MCP prompts for templates like 'draft a primitive of type X'.",
      target_version: "0.2",
    },
  },
  {
...
const sections: PrimitiveSpec[] = [
  ...
  },
  {
    ...
    fields: {
      ...
      kind: "prose",
      body_md: [
        "The MCP server holds a `Host` in memory and faces the same concurrency problem the REPL does ([SPEC-REPL.md](./SPEC-REPL.md) §7). The freshness model is identical: per-project `stat` against `JsonlLogStore` log paths, profile-directory stat for tools that read `profile_id`, and the explicit `Host.reload()` and `Host.reloadProjectTail(project_id)` methods SPEC-REPL §13 requires.",
        "",
        "Strict mode for Tier 2 / Tier 3 tools: refuse with the `permission` error envelope carrying `evidence.reason: \"stale_state\"` if any addressed project's log has changed out-of-band. Lenient mode for Tier 1: incremental tail-replay then dispatch. Operators trigger `Host.reload()` via SIGHUP or process restart.",
        "",
