 ...
 * optional scope_id, revision) for a single id within a workbook.
...
export const tool: McpToolEntry<z.infer<typeof Input>, z.infer<typeof Output>> = {
  ...
  tier: "read_only",
  description:
    "Fetch one primitive by id within a workbook. Pass `fields` to workbook a subset of top-level response keys. Throws not_found if the id is absent.",
  input: Input,
  output: Output,
