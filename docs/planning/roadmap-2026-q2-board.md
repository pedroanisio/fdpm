...
- `task:p1-knowledge-cartridge` _(Either/P1)_ — profile:knowledge-cartridge:1.0 — 13 primitives (six are the layers), 6 relations, 9 validator rule ids running the generator protocol's Pass 6, 3 renderers and 1 MCP prompt. Discarded harvest retained so the discard rate is counted; 3 unenforceable checks declared, not dropped.
- `task:p1-loop-forward-prompts` _(Either/P1)_ — fdpm.loop-forward ships loop-forward/author\_pipeline (endpoint-before-edge call order, all eight lf:val:\* failure modes) and loop-forward/audit\_pipeline (review via the five renderers). Drift gate over cited ids; 4,500 B body ceiling; no new MCP tool.
