...
- `task:fallback-detection` _(Either/P0)_ — CHG-3: Detect 'no \`order\` edges in workbook' and route through the legacy compareSectionNumbers path; emit \`info\`-level deprecation findings on mixed-mode workbooks (Section with both authored \`number\` and a derivable position).
...
- `task:tests-fallback-fixture` _(Either/P0)_ — Fallback fixture in spec\_md.test.ts: a workbook authored with the v0.0 pattern (only authored \`number\`, no \`order\` edges) renders byte-equal to its pre-renderer-change output.
- `task:tests-mixed-mode` _(Either/P1)_ — Mixed-mode fixture in spec\_md.test.ts: a workbook with both authored \`number\` and \`order\` edges produces the expected count of \`info\`-level deprecation findings — one per mixed Section.
