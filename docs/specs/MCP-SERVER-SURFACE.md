...
| `fdpm.profile.get` | fetch profile | Fetch a `DomainProfile` by id; throws `not_found` if unknown. `view` selects `full` (default) \| `summary` \| `type_ids` \| `types`; `fields` projects top-level keys. `full` is the raw, un-resolved profile as registered; the three vocabulary views report the **resolved** profile, so a composition profile's inherited types are included and every id `type_ids` returns is one `type_info` accepts. Measured over the 27 loaded profiles: `full` 448 B – 5,409,966 B (14 over the ceiling), `types` 117 B – 1,835,052 B (5 over), `type_ids` 126 B – 26,830 B (none over), `summary` 280 B – 1,045 B (none over). An oversized read is refused, and the refusal names only the views measured to fit that call |
