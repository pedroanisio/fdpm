...
[Response]          Server returns `isError: true` with `structuredContent.error.category = "permission"` and `evidence.reason = "destructive_disabled"`; no operation is appended to the log; no project state is mutated; the call is recorded in the audit log.
...
```
[Source]            LLM agent.
[Stimulus]          Calls `fdpm.primitive.search` against a representative project on the standard fixture set.
...
- **4. Stale-state refusal on concurrent CLI write** — While the MCP server is running, run a `fdpm` CLI command that mutates the project. Then issue a Tier 2 MCP call against the same project.
...
- **MCP `resources` and `prompts` surfaces** _(target: 0.2)_ — Native MCP resources for project state (not just tools) and MCP prompts for templates like 'draft a primitive of type X'.
