...
_A small, sandboxed expression language evaluated at render time. Variables, workbook-graph queries, and conditional sections — no Turing-complete escape hatches._
...
| Operator | Determinism: same workbook state + same template ⇒ byte-identical output. Required for the GENERATED-DOCUMENT banner's claims to hold. |
...
| Attribute | Pressure |
| --- | --- |
| **Determinism** | Two renders of the same workbook state MUST produce byte-identical output. Any non-determinism (system clock, env var drift, map iteration order) is a contract violation. |
...
3. (MUST) **Activation surface and helper set are closed.** `{ doc, workbook, env, host, fn }` exhaustively names the Tier-A activation (per SPEC-EXPRESSION-RUNTIME §M7). The `fn.*` helper set is enumerated in §6.4. Adding either is a SPEC amendment to SPEC-EXPRESSION-RUNTIME, not a code change here.
...
| **Variable expression** | A placeholder that resolves to a scalar value, e.g., `${doc.title}` or `${workbook.fingerprint}`. The path before the first `.` MUST name a Tier-A binding (per SPEC-EXPRESSION-RUNTIME §M7) or a Tier-B binding the plugin has opted into. |
...
- **(MUST) Queries are read-only** — Render-time CEL list-comprehension expressions and helper calls MUST never produce side effects. Iteration over `workbook.primitives` / `workbook.relations` is read-only; the DSL MUST NOT introduce mutation forms.
...
- **(MUST) Determinism across runs** — Given identical workbook state and identical template, two render invocations MUST produce byte-identical output. Implementations MUST NOT depend on Map iteration order for output assembly.
...
- **2. Bounded iteration cap fires on adversarial input** — Render a template whose CEL expression would enumerate more than the configured bound from `workbook.primitives` on a 200 000-primitive workbook.
  - expected: render-error: iteration bound exceeded, with the rendered bytes carrying inline markers under default CLI policy.
- **3. Two renders are byte-identical** — Render the SPEC-CEL-VALIDATOR workbook twice from independent fresh data dirs; diff -q.
...
| **Per-render parse cost** — Parsing each template per render dominates p50 latency on small workbooks. | Cache compiled templates by template-id at registration time. Per-render work becomes evaluate-only. |
...
- **Per-section schema scoping in the renderer** _(target: spec_authoring 0.2)_ — Today `kind: "schema"` emits every spec:SchemaDefinition in the workbook. Once two SPECs need separate schemas in distinct sections (this SPEC's grammar vs. helpers), add a `target_id` (or relation-based) scoping mechanism so each section selects its own schema.
- **Cross-workbook queries** _(target: 0.3)_ — Today `workbook` is single-bound. A future cross-workbook CEL activation surface could let one document reference another workbook's state for portfolio-level rollups, but v0.1 intentionally stays single-workbook.
