...
Candidate CLI version: `1.3.0`
Candidate bridge version: `0.4.0`

> **Status update — 2026-09-04.** The repository is now `pedroanisio/fdpm`
> (renamed from `fdpm-cli`; the old URL redirects) and the live GitHub API
> returns `visibility=public` with `license.spdx_id=Apache-2.0`, so operator
> action 2 below is done and action 3 is partly done. The maintainer chose
> the Apache License, Version 2.0 (SPDX `Apache-2.0`), committed in
> `89f4193`: `LICENSE` at the root, `fdpm-cli/LICENSE`, and
> `fdpm-cli/packages/zod-bridge/LICENSE` are byte-identical copies of the
> canonical apache.org text (SHA-256
...
> longer block. `fdpm-cli/package.json` is now `1.3.0`, aligned with
> `HOST_VERSION` in `src/core/version/spec.ts`, which removes the cause of
> the version-identity finding `npm run public:check` reported; re-run on
> 2026-09-04 after the bump, `npm run public:check` printed
> "Public-readiness check passed." and `npm audit --omit=dev
> --audit-level=high` found 0 vulnerabilities. `@fdpm/cli` and `@fdpm/zod-bridge`
> remain unpublished (the registry returned 404 for both on 2026-09-04), so
> "npm availability" is the one remaining external gate. The rest of this
> document is the 2026-08-30 assessment, preserved as written.
